...
function makeSupportFeature(mcData) {
	return (feature) =>
		features.some(({ name, versions }) => name === feature && versions.includes(mcData.version.majorVersion))
}
	...
	function moveEntityWithHeading(entity, world, strafe, forward) {
		...
		if (entity.isInWater || entity.isInLava) {
			...
			if (
				...
				(entity.isCollidedHorizontally || (supportFeature("climbUsingJump") && entity.control.jump))
			) {
		...
		for (cursor.y = Math.floor(bb.minY); cursor.y <= Math.floor(bb.maxY); cursor.y++) {
			for (cursor.z = Math.floor(bb.minZ); cursor.z <= Math.floor(bb.maxZ); cursor.z++) {
				for (cursor.x = Math.floor(bb.minX); cursor.x <= Math.floor(bb.maxX); cursor.x++) {
					...
					if (
						block &&
						(waterIds.includes(block.type) || waterLike.has(block.type) || block.getProperties().waterlogged)
					) {
